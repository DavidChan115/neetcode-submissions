...
# Time complexity: one for loop, two strings, so the complexity is the length of these two strings O(n+m)

# Space complexity: 
# Two dicts. Each dict stores one entry per unique char, and the value is just an int. So extra memory is proportional to how many different letters you saw, not to the length of the strings 
# 呢條題目話alphabet得細階，即係得26個字母，26係constant，所以係O(1)
# 理論上如果大細階又計，符號又計，unicode又計...個dict儲存嘅entry可以到n個, space complexity就會係O(n), 即係gurantee唔到O(1) 
