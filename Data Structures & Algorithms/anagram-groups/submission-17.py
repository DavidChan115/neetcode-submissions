...
# time complexity: nested for loop, suppose there are m strings in strs, and then n is the length of the longest string within strs, then n will be bottleneck, so nested loop = O(m * n)

# space complexity: There are two ways of calculating this:
#   1. if you don't count the list returning as the result, then the dictionary only contains m number of keys, and all there keys are constant size of 26, so the time complexity will only be O(m)
#   2. if you count the list returning as the result, there are m number of strings, Each of those strings can be up to length n, so the total amount of character data sitting inside the returned structure is O(m * n)